...
public class AemetPrevisionAdapter implements PrevisionRepository {
    ...
    public PrevisionDiaResponse buscarPrevisionMunicipio(String id, String unidades) {
        final String baseUrlPrediccion = UtilsUri.buildUrl(aemetProperties.getUrlBase(), aemetProperties.getPathPrediccionHorariaMunicipio());
        final String urlPrediccion = UtilsUri.buildPathSegment(baseUrlPrediccion, MunicipioUtils.obtenerCodigoMunicipio(id));
        final AemetResponse aemetResponse = aemetRestClient.get(urlPrediccion, AemetResponse.class);
        ...
        List<PrediccionMunicipioDto> municipioResponse = this.aemetRestClient.get(aemetResponse.getDatos(), new ParameterizedTypeReference<>() {});
